...
int main()
{
    ...
    printf("%s\n", PesquisaSequencial_2(Numeros, Tamanho, Elemento));
...
int PesquisaSequencial(int * V, int N, int Chave)
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        if (V[i] == Chave)
        {
            return i; // O elemento chave foi encontrado no valor
        }
    }
    return ERRO;
}
 ...
 char Resposta[100]
    for (i = 0; i < N; i++)
    {
        ...
        {   
            ...
            return Resposta; // O elemento chave foi encontrado no valor
        }
    }
    return ERRO;
